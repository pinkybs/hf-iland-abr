    ...
    public static function getSearchFriend()
    {
		...
		if($friendList){
			foreach($friendList as $k1=>$v1){
        		...
        		if ($user['figureurl'] == 'http://avatars.nasza-klasa.pl/img/avatar/brakzdjecia') {
                    $user['figureurl'] = STATIC_HOST.'/apps/island/images/common/nopic.gif';
                }
				$list[$k1]['uid'] = $v1;
				$list[$k1]['name'] = $user['name'];
				$list[$k1]['face'] = $user['figureurl'];
				$list[$k1]['islandLevel'] = $userVO['level'];
				$list[$k1]['link'] = 'http://www.facebook.com/profile.php?id=' . $puser['puid'];
			}
		}
		return $list;
    }
