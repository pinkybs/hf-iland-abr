 ...
 */
class Bll_Island_Task_Build_T2000 extends Bll_Abstract implements Bll_Island_Task_Interface
{
    /**
     * check user task
     *
    ...
    public function check($uid, $taskId)
    {
        ...
        if ( $taskInfo['need_num'] == 1 ) {
            ...
            $itemLevel = $dalPlant->getUserPlantByItemId($uid, $taskInfo['item_id']);
        	if ( $itemLevel < $taskInfo['item_level'] ) {
                $result['content'] = 'serverWord_150';
                return $result;
            }
        }
        else if ( $taskInfo['need_num'] > 1 ) {
            $plantList = $dalPlant->getPlantListByItemId($uid, $taskInfo['item_id']);
            $plantCount = 0;
            foreach ($plantList as $plant) {
                if ( $plant['level'] >= $taskInfo['item_level'] ) {
                    $plantCount += 1;
                }
            }
