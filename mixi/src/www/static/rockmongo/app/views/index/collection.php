...
<h3><a href="<?php h(url("databases"));?>">Databases</a> &raquo; <a href="<?php h(url("db",array("db"=>$db)));?>"><?php h($db);?></a> &raquo; <?php h($collection);?></h3>

<div class="operation"><strong>Query</strong>[<a href="<?php h($arrayLink);?>" <?php if(x("format")=="array"):?>style="text-decoration:underline"<?php endif;?>>Array</a>|<a href="<?php h($jsonLink); ?>" <?php if(x("format")!="array"):?>style="text-decoration:underline"<?php endif;?>>JSON</a></a>] | <a href="<?php h(url("createRow", xn())); ?>">Insert</a> | <a href="<?php h($_SERVER["REQUEST_URI"]);?>">Refresh</a> | <a href="<?php h(url("collectionIndexes", xn())); ?>">Indexes</a> | <a href="<?php h(url("collectionStats", xn())); ?>">Statistics</a> | <a href="<?php h(url("clearRows", xn())); ?>" onclick="return window.confirm('Are you sure to delete all records in collection \'<?php h($collection);?>\'?');">Clear</a> | <a href="<?php h(url("removeCollection", xn())); ?>" onclick="return window.confirm('Are you sure to drop collection \'<?php h($collection);?>\'?');">Drop</a> </div>
