...
class Hapyfish2_Island_Event_Bll_TeamBuy
{
	...
	public static function checkIcon($uid)
	{
		...
		if($info['start_time'] + ($info['ok_time'] + $info['buy_time']) * 3600 - time() < 0) {
			...
			$checkkey = 'i:e:teambuy:check:' . $uid;
			$checkVo = $newcache->get($checkkey);

			if ($checkVo == false) {
				$newcache->delete($newkey);
				$newcache->set($checkkey, 1);
			}
...
//		}
     ...
     * @param return array
     */
    public static function buyGoodsTest($uid)
    {
        $result = array('status' => -1);
        ...
        //调试
        $result['testAry_1'] = $info;
        $result['testAry_state'] = $state;
        
        $stateTest = $mcache->get($mkey);
    
        //判断用户的状态:$attendflag:-1:已加入;$buyokflag:1-已购买,-1:加入没有购买),
        if(!$stateTest) {
            $result['testAry_state_test_1'] = 1;
            $attendflag = -1;
            $buyokflag = -1;
        } else {
            $result['testAry_state_test_1'] = 2;
            $attendflag = 1;
            $buyokflag = $stateTest;

            //$mcache->set($mkey, $stateTest);
        }
        $result['testAry_state_test'] = $stateTest;

        if($state === false) {
        	
            $result['testAry_state_1'] = $state;
            try {
                $state = $dalDB->getJoinTeamBuyInfo($uid);
                
                $result['testAry_state_2'] = $state;
            } catch (Exception $e) {
                info_log($e, 'teambuy_info');
