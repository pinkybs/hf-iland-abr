...
class Hapyfish2_Island_Event_Bll_Hash
{
	...
	public static function getGift($uid)
	{
		...
		if(!$switch){
			$rsp = array ();
			$rsp ['result'] ['status'] = - 1;
			$rsp ['result'] ['content'] = 'Sự kiện chưa mở!';
			return $rsp;
		}

		$rsp = array ();
		$rsp ['result'] ['status'] = - 1;
		$rsp ['result'] ['content'] = 'Bạn đã nhận thưởng, mau tới kho xem!';
		...
		if( !$val ) {
	    	...
	    	if( $i == 5 ) {
				...
				if($ok) {
					$title = 'Chúc mừng bạn nhận <font color="#FF0000">'.$name.'</font>, mau xem thử!';
