...
class ConsumelogsController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        $info = $this->vailid();
        if (!$info) {
        	echo '<html><body><script type="text/javascript">window.top.location="http://http://soap.soha.vn/apps/dtd";</script></body></html>';
        	exit;
        }
    ...
    public function coinAction()
    {
    	...
    	$pageSize = 25;
    	$this->view->user = $user;
    	$this->view->date = $year . 'Năm' . $month . 'Tháng';
		$this->view->logs = $logs;
    ...
    public function goldAction()
    {
    	...
    	$pageSize = 15;
    	$this->view->user = $user;
    	$this->view->date = $year . 'Năm' . $selMonth . 'Tháng';
