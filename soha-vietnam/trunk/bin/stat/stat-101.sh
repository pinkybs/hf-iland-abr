...
/usr/local/mysql/bin/mysql -u worker -p'XEJCHfigOJ' -h 192.168.7.18  ${statdb}  -e "update day_main set active=${num4},active_male=${num5},active_male=${num5},active_female=${num6} where log_time=${date1}"
...
/usr/local/mysql/bin/mysql -u worker -p'XEJCHfigOJ' -h 192.168.7.18 ${statdb} -e "insert into  day_active_user_level  values('${date1}','${level}')"

######################################

/usr/local/mysql/bin/mysql -u worker -p'XEJCHfigOJ' -h 192.168.7.18 ${statdb} -e "delete from day_user_retention where log_time=${date1}"

/usr/local/mysql/bin/mysql -u worker -p'XEJCHfigOJ' -h 192.168.7.18 ${statdb} -e "insert into day_user_retention(log_time) values(${date1})"
...
do
    ...
    /usr/local/mysql/bin/mysql -u worker -p'XEJCHfigOJ' -h 192.168.7.18 ${statdb} -e "update day_user_retention set day_${n}=${k} where log_time=${date1}"
done
