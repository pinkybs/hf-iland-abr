...
class osapiRestIO extends osapiIO {
  ...
  private static $urlTemplates = array(
    ...
    // Mixi Specific
    'mixipoint' => 'atom/mobile/point/{userId}',
    'classmates' => 'classmates/{userId}/{schoolId}'
  );
    ...
    foreach ($requests as $request) {
      ...
      if (!$entry instanceof osapiError) {
          if ($request->method == 'mixipoint.create') {
            ...
            // update by pcplayboy 2009/06/12
            $dataCount = 0;
            if (isset($entry['data']['list'])) {
              if (isset($entry['data']['totalResults'])) {
                $dataCount = $entry['data']['totalResults'];
              } else {
                $dataCount = count($entry['data']);
              }
            }
            if ($dataCount != 1) {
              ...
              if (isset($entry['data']['list'])) {
                if(count($entry['data']['list']) == 1) {
                    $entry['data'] = self::convertArray($request, $entry['data']['list'][0], $strictMode);
                } else {
  ...
  private static function executeRestRequest(osapiRequest $request, osapiProvider $provider, osapiAuth $signer) {
    ...
    if ($method != 'GET') {
      if (isset(self::$postAliases[$service]) && isset($request->params[self::$postAliases[$service]])) {
      	...
      	} else if ($request->method == 'mixipoint.create') {
    ...
    if (method_exists($provider, 'getRestEndpoint')) {
        $baseUrl = $provider->getRestEndpoint($request, $method);
    } else {
    ...
    if ($hasPostBody) {
        if($request->method == 'mediaItems.upload') {
            ...
            // Mixi mobile point use 'ATOM' format
            $postBody = $request->params[self::$postAliases[$service]]->toAtomString();
        } else {
            // Pull out the (possibly) modified post body parameter and
            // unset it from the request, so that it doesn't get signed.
            //update pcplayboy 2009-09-24
            if (method_exists($provider, 'jsonEncode')) {
                $postBody = $provider->jsonEncode($request->params[self::$postAliases[$service]]);
            }else {
