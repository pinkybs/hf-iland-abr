...
class osapiOAuth2Legged extends osapiAuth {
  ...
  /*
   * add some paramters for mixi
   * and public set method:
   * setSignBody, setSendHeaderAuthorization
   */
  protected $signBody;
  protected $sendHeaderAuthorization;
  ...
  public function __construct($consumerKey, $consumerSecret, $userId = null) {
    ...
    $this->signBody = true;
    $this->sendHeaderAuthorization = false;
  }
  ...
  /**
   * Set whether to sign body in oauth_signature.
   * @param bool $value True if body should be signed in oauth_signature.
   */
  public function setSignBody($value) {
    $this->signBody = $value;
  }
  
  /**
   * Set whether to send Authorization information in header.
   * @param bool $value True if Authorization should be send in header.
   */
  public function setSendHeaderAuthorization($value) {
    $this->sendHeaderAuthorization = $value;
  } 
  ...
  public function sign($method, $url, $params = array(), $postBody = false, &$headers = array()) {
    ...
    if (!$this->sendHeaderAuthorization || $method == 'GET') {
        $signedUrl = $oauthRequest->to_url();
    }
    else {
        $p = $oauthRequest->get_parameters();
        $total = array();
        foreach ($p as $k => $v) {
        ...
        $signedUrl = $oauthRequest->get_normalized_http_url() . "?" . $out;
