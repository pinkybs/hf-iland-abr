...
class Hapyfish2_Platform_Bll_User
{
    ...
    public static function getVUID($uid, $session_key = null, $puid = null)
    {
    	$vuid = Hapyfish2_Platform_Cache_User::getVUID($uid);
    	if (empty($vuid)) {
    		$rest = Kaixin_Rest::getInstance();
    		$rest->setUser($puid, $session_key);
    		$verify = $rest->getEncodeSessionKey();
    		if ($verify) {
    			$tmp = explode('_', $verify);
    			if (count($tmp) > 1) {
    				$vuid = $tmp[0];
    				Hapyfish2_Platform_Cache_User::updateVUID($uid, $vuid);
    			}
    		}
    	}
